...
{
  [super viewDidLoad];
    [self createFirebaseAndAuthClient];
...
- (void)createFirebaseAndAuthClient
{
    NSLog(@"Creating Firebase...");
    
    if (self.firebase == nil) {
        NSLog(@"Firebase in the sliding controller...");
        self.firebase = [[Firebase alloc] initWithUrl:@"https://small-talk.firebaseIO.com/"];
    }
    
    if (self.authClient == nil) {
        NSLog(@"AuthClient in the sliding controller...");
        self.authClient = [[FirebaseAuthClient alloc] initWithRef:self.firebase];
    }
}
